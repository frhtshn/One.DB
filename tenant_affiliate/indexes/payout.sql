...
CREATE INDEX idx_payouts_request ON payout.payouts USING btree(payout_request_id) WHERE payout_request_id IS NOT NULL;
...
CREATE INDEX idx_payouts_period ON payout.payouts USING btree(period_start, period_end);
CREATE INDEX idx_payouts_pending ON payout.payouts USING btree(status, created_at) WHERE status IN (0, 1, 2);
CREATE INDEX idx_payouts_processed_by ON payout.payouts USING btree(processed_by) WHERE processed_by IS NOT NULL;

-- payout_commissions (junction table)
CREATE INDEX idx_payout_commissions_payout ON payout.payout_commissions USING btree(payout_id);
CREATE INDEX idx_payout_commissions_commission ON payout.payout_commissions USING btree(commission_id);
CREATE UNIQUE INDEX idx_payout_commissions_unique ON payout.payout_commissions USING btree(payout_id, commission_id);
