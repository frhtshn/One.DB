...
-- tenant_affiliate veritabanı için retention yönetimi
-- Varsayılan: Sınırsız retention (business data)
...
BEGIN
    ...
    FOR v_rec IN
        ...
        WHERE pn.nspname = 'tracking'
          AND cc.relname NOT LIKE '%_default'
        ORDER BY cc.relname
    LOOP
        -- Partition adından tarihi çıkar (format: tablo_yYYYYmMM veya tablo_yYYYYmMMdDD)
        BEGIN
...
COMMENT ON FUNCTION maintenance.drop_expired_partitions(INT) IS 'Drops monthly partitions older than retention period. Default: indefinite (business data). Never drops current month.';
