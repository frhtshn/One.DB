...
CREATE OR REPLACE FUNCTION maintenance.drop_expired_partitions(
    p_retention_days INT DEFAULT 7  -- 7 gün varsayılan retention
)
RETURNS TABLE (
    table_name TEXT,
    partition_name TEXT,
...
COMMENT ON FUNCTION maintenance.drop_expired_partitions(INT) IS 'Drops daily partitions older than retention period (default 7 days) for game_log tables. Never drops today.';
