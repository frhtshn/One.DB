-- ================================================================
-- GAME_PROVIDER_SYNC: Core DB'den game provider senkronizasyonu
-- ================================================================
-- Backend, Core DB'den GAME tipli provider'ları alıp
-- bu fonksiyona TEXT (JSONB array) olarak geçirir.
-- Her eleman için UPSERT yapılır (id bazlı).
-- ================================================================

DROP FUNCTION IF EXISTS catalog.game_provider_sync(TEXT);

CREATE OR REPLACE FUNCTION catalog.game_provider_sync(
    p_providers TEXT
)
RETURNS INTEGER
LANGUAGE plpgsql
...
BEGIN
    ...
    LOOP
        INSERT INTO catalog.game_providers (
            id,
            provider_code,
            provider_name,
            is_active,
...
COMMENT ON FUNCTION catalog.game_provider_sync(TEXT) IS 'Syncs game-type providers from Core DB. Accepts TEXT→JSONB array, performs id-based UPSERT. Returns number of upserted providers.';
