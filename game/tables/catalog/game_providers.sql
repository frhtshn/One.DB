-- =============================================
-- Tablo: catalog.game_providers
-- Açıklama: Game-type provider referans tablosu
-- Core DB'deki catalog.providers tablosunun
-- GAME tipli alt kümesinin hafif kopyası.
-- Aynı ID'ler kullanılır (BIGINT PK, SERIAL değil).
-- Backend game_provider_sync ile senkronize edilir.
-- =============================================

DROP TABLE IF EXISTS catalog.game_providers CASCADE;

CREATE TABLE catalog.game_providers (
    id BIGINT PRIMARY KEY,                                     -- Core catalog.providers.id ile aynı (SERIAL değil)
    provider_code VARCHAR(50) NOT NULL,                        -- Provider kodu: PRAGMATIC, EVOLUTION, EGT
    provider_name VARCHAR(255) NOT NULL,                       -- Görünen ad: Pragmatic Play, Evolution Gaming
    is_active BOOLEAN NOT NULL DEFAULT true,                   -- Aktif/pasif durumu
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),             -- Oluşturulma zamanı
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()              -- Güncellenme zamanı
);

COMMENT ON TABLE catalog.game_providers IS 'Lightweight reference of game-type providers synced from Core DB. Uses same IDs as catalog.providers for cross-DB consistency.';
