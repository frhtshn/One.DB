-- =============================================
-- Tablo: catalog.games
-- Açıklama: Oyun kataloğu (Game DB — bounded context owner)
-- Tüm provider'lardan gelen oyunların merkezi listesi.
-- Core DB'den taşındı — Game DB catalog'un sahibidir.
-- Gateway API sync veya BO admin CSV/Excel import ile doldurulur.
-- Her oyun tek bir game_provider'a bağlıdır.
...
CREATE TABLE catalog.games (
    ...
    provider_id BIGINT NOT NULL,                                    -- Oyun sağlayıcı ID (FK: catalog.game_providers)
...
COMMENT ON TABLE catalog.games IS 'Master game catalog (Game DB bounded context owner). Synchronized from integrated game providers via gateway API or BO admin import. Contains all game metadata, features, and configuration.';
