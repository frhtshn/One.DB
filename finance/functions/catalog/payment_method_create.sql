...
-- Provider varlığı catalog.payment_providers'dan kontrol edilir.
-- Aynı provider içinde code unique olmalıdır.
-- Core DB'den taşındı — Finance DB catalog'un sahibidir.
-- supported_cryptocurrencies desteği eklendi.
-- ================================================================
...
DROP FUNCTION IF EXISTS catalog.payment_method_create(
    ...
    DECIMAL, DECIMAL, DECIMAL, DECIMAL,
    VARCHAR, VARCHAR, CHAR(3)[], VARCHAR(20)[], CHAR(2)[], SMALLINT, BOOLEAN, BOOLEAN,
...
BEGIN
    ...
    -- Provider varlık kontrolü (Finance DB'deki payment_providers)
    IF NOT EXISTS(SELECT 1 FROM catalog.payment_providers pp WHERE pp.id = p_provider_id) THEN
    ...
    INSERT INTO catalog.payment_methods (
        ...
        supported_currencies, supported_cryptocurrencies, blocked_countries,
    ...
    VALUES (
        ...
        p_supported_currencies, p_supported_cryptocurrencies, p_blocked_countries,
...
COMMENT ON FUNCTION catalog.payment_method_create IS 'Creates a new payment method in Finance DB catalog. Validates provider exists in payment_providers. Code must be unique per provider. Supports fiat and crypto currencies.';
