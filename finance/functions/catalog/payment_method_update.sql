...
-- Provider değiştiriliyorsa catalog.payment_providers'dan kontrol edilir.
...
BEGIN
    ...
    -- Provider değiştiriliyorsa varlık kontrolü (Finance DB'deki payment_providers)
    ...
    SET provider_id = COALESCE(p_provider_id, pm.provider_id),
        ...
        supported_cryptocurrencies = COALESCE(p_supported_cryptocurrencies, pm.supported_cryptocurrencies),
...
COMMENT ON FUNCTION catalog.payment_method_update IS 'Updates a payment method in Finance DB catalog. COALESCE pattern preserves existing values for NULL params. Validates provider and code uniqueness. Supports fiat and crypto currencies.';
