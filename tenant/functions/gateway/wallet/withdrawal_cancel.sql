-- ================================================================
-- WITHDRAWAL_CANCEL: Para çekme iptal et (REVERSAL)
-- ================================================================
-- BO red veya oyuncu iptali sonrası çağrılır.
-- Reversal transaction oluşturarak bakiyeyi geri ekler.
-- Zaten onaylanmış withdrawal iptal edilemez.
-- Auth-agnostic (backend çağırır).
-- ================================================================

DROP FUNCTION IF EXISTS wallet.withdrawal_cancel(VARCHAR, VARCHAR);

CREATE OR REPLACE FUNCTION wallet.withdrawal_cancel(
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION USING ERRCODE = 'P0404', MESSAGE = 'error.withdrawal-cancel.already-confirmed';
    END IF;

    -- Zaten onaylanmış → iptal edilemez
    IF v_tx.confirmed_at IS NOT NULL THEN
        RAISE EXCEPTION USING ERRCODE = 'P0409', MESSAGE = 'error.withdrawal-cancel.already-confirmed';
    END IF;

    -- Zaten iptal edilmiş → idempotent dönüş kontrolü
    IF v_tx.processed_at IS NOT NULL THEN
        -- Reversal tx'i bul ve sonucunu dön
    ...
    -- Orijinal tx'i iptal olarak işaretle
    UPDATE transaction.transactions
    SET processed_at = NOW(),
        description = 'CANCELLED: ' || COALESCE(p_reason, 'No reason provided')
    WHERE id = v_tx.id
    ...
    INSERT INTO transaction.transactions (
        ...
        v_tx.id,
        'PAYMENT', 'Withdrawal cancellation reversal: ' || COALESCE(p_reason, ''),
...
COMMENT ON FUNCTION wallet.withdrawal_cancel IS 'Cancels a pending withdrawal by creating a reversal transaction that credits the wallet back. Cannot cancel an already confirmed withdrawal.';
