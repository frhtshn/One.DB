-- ================================================================
-- BET_PROCESS: Bahis işlemi (DEBIT)
-- ================================================================
-- Seamless wallet gateway'in en kritik fonksiyonu.
-- REAL wallet'tan bahis tutarını düşer. FOR UPDATE ile satır
-- kilidi, idempotency_key ile retry-safe. Provider-agnostic.
-- Auth-agnostic (backend çağırır).
-- ================================================================

DROP FUNCTION IF EXISTS wallet.bet_process(BIGINT, VARCHAR, DECIMAL, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, SMALLINT, TEXT);

CREATE OR REPLACE FUNCTION wallet.bet_process(
    p_player_id BIGINT,
    ...
    p_transaction_type_id SMALLINT DEFAULT 3,
...
BEGIN
    ...
    -- Bonus bakiye oku (response için, FOR UPDATE kullanılmaz)
    SELECT COALESCE(ws.balance, 0) INTO v_bonus_balance
    FROM wallet.wallets w
    JOIN wallet.wallet_snapshots ws ON ws.wallet_id = w.id
    WHERE w.player_id = p_player_id
...
COMMENT ON FUNCTION wallet.bet_process IS 'Processes a bet by debiting the REAL wallet. Idempotent via idempotency_key. Uses FOR UPDATE row lock for concurrency safety. Returns transaction ID and updated balances.';
