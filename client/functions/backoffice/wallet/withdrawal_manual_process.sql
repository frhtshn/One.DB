-- ================================================================
-- WITHDRAWAL_MANUAL_PROCESS: Manuel para çekme (tek adım)
-- ================================================================
-- BO admin tarafından kullanılır. PSP gerektirmez, workflow
-- gerektirmez. Tek adımda wallet'tan debit yapar.
-- İdempotent: aynı key ile tekrar çağrılırsa mevcut sonucu döner.
-- Auth-agnostic (backend çağırır).
-- ================================================================

DROP FUNCTION IF EXISTS wallet.withdrawal_manual_process(BIGINT, VARCHAR, DECIMAL, VARCHAR, VARCHAR, TEXT);

CREATE OR REPLACE FUNCTION wallet.withdrawal_manual_process(
    p_player_id       BIGINT,
...
COMMENT ON FUNCTION wallet.withdrawal_manual_process IS 'Processes a manual withdrawal in a single step. Debits the wallet immediately with confirmed transaction. No PSP or workflow required.';
