...
-- finance_log veritabanı için retention yönetimi
-- Aktif (bugünün) partition'ını ASLA silmez
-- Varsayılan retention: 14 gün
-- ================================================================
...
CREATE OR REPLACE FUNCTION maintenance.drop_expired_partitions(
    p_retention_days INT DEFAULT 14  -- Varsayılan retention: 14 gün
)
RETURNS TABLE (
    table_name TEXT,
    partition_name TEXT,
...
BEGIN
    ...
    FOR v_rec IN
        ...
        WHERE pn.nspname IN ('finance_log')
...
COMMENT ON FUNCTION maintenance.drop_expired_partitions(INT) IS 'Drops daily partitions older than retention period (default 14 days). Never drops current day partition. Safety-first design.';
