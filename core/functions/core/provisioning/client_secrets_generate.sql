-- ================================================================
-- CLIENT_SECRETS_GENERATE: Secret placeholder kayıtları oluştur
-- ================================================================
...
DROP FUNCTION IF EXISTS core.client_secrets_generate(BIGINT, VARCHAR);

CREATE OR REPLACE FUNCTION core.client_secrets_generate(
    p_client_id BIGINT,
...
BEGIN
    ...
    FOREACH v_type IN ARRAY v_secret_types LOOP
        IF NOT EXISTS (
            SELECT 1 FROM security.secrets_client
            WHERE client_id = p_client_id
              AND secret_type = v_type
              AND environment = p_environment
        ) THEN
            INSERT INTO security.secrets_client (
                client_id, secret_type, secret_value, environment, is_active, created_at
            ) VALUES (
                p_client_id,
                v_type,
...
COMMENT ON FUNCTION core.client_secrets_generate(BIGINT, VARCHAR) IS 'Creates placeholder secret entries (JWT_SECRET, ENCRYPTION_KEY, API_KEY) for a client. Real values are generated and written by ProductionManager. Idempotent via NOT EXISTS check.';
