-- ================================================================
-- TENANT_PROVIDER_DISABLE: Tenant provider kapatma
-- ================================================================
-- Sadece flag günceller (is_enabled=false).
-- Oyunlara (core.tenant_games) DOKUNMAZ.
-- Provider durumu sorgu seviyesinde filtrelenir.
-- ================================================================

DROP FUNCTION IF EXISTS core.tenant_provider_disable(BIGINT, BIGINT, BIGINT);

CREATE OR REPLACE FUNCTION core.tenant_provider_disable(
    p_caller_id BIGINT,
    p_tenant_id BIGINT,
...
COMMENT ON FUNCTION core.tenant_provider_disable IS 'Disables a provider for tenant (flag only). Games remain untouched - provider status is filtered at query level.';
