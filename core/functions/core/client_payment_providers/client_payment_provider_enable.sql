-- ================================================================
-- TENANT_PAYMENT_PROVIDER_ENABLE: Tenant'a payment provider aç + metotları seed et
-- ================================================================
...
DROP FUNCTION IF EXISTS core.tenant_payment_provider_enable(BIGINT, BIGINT, BIGINT, TEXT, VARCHAR, VARCHAR);

CREATE OR REPLACE FUNCTION core.tenant_payment_provider_enable(
    p_caller_id BIGINT,
    p_tenant_id BIGINT,
...
COMMENT ON FUNCTION core.tenant_payment_provider_enable IS 'Enables a PAYMENT provider for tenant and seeds payment methods from backend-provided data (cross-DB orchestration). Existing methods untouched (ON CONFLICT DO NOTHING). Supports shadow rollout mode.';
