...
-- =============================================
-- Unified user_roles: client_id = NULL for global, client_id = değer for client
-- Erişim:
...
--   - CompanyAdmin: clientadmin + altı (kendi şirketinde)
--   - ClientAdmin: moderator + altı (kendi client'ında)
-- Kısıtlamalar:
...
DECLARE
    ...
    v_role_level INT;
    v_role_is_client_role BOOLEAN;
    v_role_code VARCHAR;
    v_already_assigned BOOLEAN;
BEGIN
    -- 1. Caller bilgisi (global + hedef client rolleri dikkate alınır)
    SELECT
        u.company_id,
        ...
        EXISTS(
            SELECT 1 FROM security.user_roles ur2
            JOIN security.roles r2 ON ur2.role_id = r2.id
            WHERE ur2.user_id = u.id AND ur2.client_id IS NULL AND r2.code = 'superadmin'
        )
    ...
    INTO v_role_id, v_role_status, v_role_level, v_role_is_client_role
    FROM security.roles
    ...
    -- 6. Rol tipi ve client_id uyumluluk kontrolü
    IF v_role_is_client_role AND p_client_id IS NULL THEN
        RAISE EXCEPTION USING ERRCODE = 'P0400', MESSAGE = 'error.role.client-required';
    END IF;

    IF NOT v_role_is_client_role AND p_client_id IS NOT NULL THEN
        RAISE EXCEPTION USING ERRCODE = 'P0400', MESSAGE = 'error.role.global-only';
    END IF;

    -- 7. Client kontrolü (varsa)
    IF p_client_id IS NOT NULL THEN
        SELECT company_id INTO v_client_company_id
        FROM core.clients
        WHERE id = p_client_id AND status = 1;
    ...
    IF NOT v_caller_is_platform_admin THEN
        ...
        -- Client ataması için client'ın aynı şirkette olması gerekir
        IF p_client_id IS NOT NULL AND v_client_company_id != v_caller_company_id THEN
    ...
    IF NOT v_already_assigned THEN
        INSERT INTO security.user_roles (user_id, role_id, client_id, assigned_at, assigned_by)
        VALUES (p_user_id, v_role_id, p_client_id, NOW(), COALESCE(p_assigned_by, p_caller_id));
    END IF;
...
Unified user_roles: client_id=NULL for global, client_id=value for client.
Access: Platform Admin (global roles), CompanyAdmin (client roles in own company).
