...
BEGIN
    ...
    WHERE code = p_code AND status = 'active' AND is_active = TRUE;
...
COMMENT ON FUNCTION messaging.message_template_get_by_code(VARCHAR, CHAR) IS 'Get active message template content by code and language. Used by backend for rendering. No auth check.';
