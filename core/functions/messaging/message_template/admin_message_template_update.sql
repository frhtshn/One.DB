...
DROP FUNCTION IF EXISTS messaging.admin_message_template_update(BIGINT, INTEGER, VARCHAR, VARCHAR, TEXT, JSONB, VARCHAR, JSONB);
...
BEGIN
    ...
    -- Kategori doğrulama
    IF p_category IS NOT NULL AND p_category NOT IN ('transactional', 'notification', 'system') THEN
...
COMMENT ON FUNCTION messaging.admin_message_template_update(BIGINT, INTEGER, VARCHAR, VARCHAR, TEXT, JSONB, VARCHAR, JSONB) IS 'Update a platform message template. Translations are replaced entirely. Channel type is immutable.';
