...
DROP FUNCTION IF EXISTS messaging.admin_message_template_list(BIGINT, VARCHAR, VARCHAR, VARCHAR, VARCHAR, INTEGER, INTEGER);
...
COMMENT ON FUNCTION messaging.admin_message_template_list(BIGINT, VARCHAR, VARCHAR, VARCHAR, VARCHAR, INTEGER, INTEGER) IS 'List platform message templates with channel, category, status, and search filters. Returns paginated results with total count.';
