...
DROP FUNCTION IF EXISTS messaging.admin_message_template_create(BIGINT, VARCHAR, VARCHAR, VARCHAR, VARCHAR, TEXT, JSONB, BOOLEAN, JSONB);
...
CREATE OR REPLACE FUNCTION messaging.admin_message_template_create(
    ...
    p_category      VARCHAR(30),                     -- Kategori: transactional, notification, system
...
BEGIN
    ...
    IF p_category IS NULL OR p_category NOT IN ('transactional', 'notification', 'system') THEN
        RAISE EXCEPTION 'error.notification-template.invalid-category';
    END IF;

    -- Kod benzersizlik kontrolü
    IF EXISTS (SELECT 1 FROM messaging.message_templates WHERE code = p_code AND is_active = TRUE) THEN
    ...
    INSERT INTO messaging.message_templates (
        ...
        p_code, p_name, p_channel_type, p_category, p_description, p_variables, COALESCE(p_is_system, FALSE), p_caller_id, p_caller_id
    )
...
COMMENT ON FUNCTION messaging.admin_message_template_create(BIGINT, VARCHAR, VARCHAR, VARCHAR, VARCHAR, TEXT, JSONB, BOOLEAN, JSONB) IS 'Create a new platform message template with multilingual translations. Validates channel-specific content requirements.';
