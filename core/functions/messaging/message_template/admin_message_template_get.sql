...
DROP FUNCTION IF EXISTS messaging.admin_message_template_get(BIGINT, INTEGER);
...
COMMENT ON FUNCTION messaging.admin_message_template_get(BIGINT, INTEGER) IS 'Get platform message template details with all translations as a single JSON response.';
