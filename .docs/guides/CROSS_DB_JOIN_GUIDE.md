...
flowchart TD
    subgraph core["Core DB (Master Data)"]
        ...
        tgames["core.client_games"]
        tpay["core.client_payment_methods"]
...
## 2. Game Kataloğu (Core ↔ Game → Client)

| Akış | Tablo | Kopyalanan Alanlar |
|---|---|---|
| Game → **Core** | `core.client_games` | `game_name`, `game_code`, `provider_code`, `game_type`, `thumbnail_url` |
| Game + Core → **Client** | `game.game_settings` | `game_code`, `game_name`, `provider_code`, `studio`, `game_type`, `game_subtype`, `categories[]`, `tags[]`, `rtp`, `volatility`, `max_multiplier`, `paylines`, `thumbnail_url`, `features[]`, `has_demo`, `has_jackpot`, `is_mobile`, `is_desktop` + client override (`custom_name`, `custom_thumbnail_url`, `rollout_status`) |
| Game + Core → **Client** | `game.game_limits` | `currency_code`, `currency_type` |

---

## 3. Payment Kataloğu (Core ↔ Finance → Client)

| Akış | Tablo | Kopyalanan Alanlar |
|---|---|---|
| Finance → **Core** | `core.client_payment_methods` | `payment_method_name`, `provider_code`, `payment_type`, `icon_url` |
| Finance + Core → **Client** | `finance.payment_method_settings` | `payment_method_code`, `payment_method_name`, `provider_code`, `payment_type`, `payment_subtype`, `channel`, `icon_url`, `logo_url`, `features[]`, `supports_recurring`, `supports_tokenization` + client override (`custom_name`, `custom_icon_url`, `rollout_status`) |
| Finance + Core → **Client** | `finance.payment_method_limits` | `currency_code`, `currency_type` |
...
| Hedef DB | Tablolar |
|---|---|
| **Client** | `wallet.wallets`, `finance.currency_rates*`, `finance.crypto_rates*`, `finance.payment_player_limits`, `finance.player_financial_limits`, `transaction.transactions`, `bonus.bonus_awards` |
| **Client Report** | Tüm 5 tablo (`currency` alanı) |
| **Core Report** | 4/5 tablo (`currency` alanı) |
| **Client Affiliate** | ~15 tablo (`currency` alanı: commissions, payouts, stats, tracking) |
| **Client Log** | `game_rounds`, `commission_calculations` |
...
| **Client Audit** | `player_audit.login_attempts`, `login_sessions` | 22 GeoIP alanı |
| **Client Audit** | `affiliate_audit.login_attempts`, `login_sessions` | 22 GeoIP alanı |
...
|---|---|---|
| **Client Log** | `game_log.game_rounds` | `game_code`, `game_name`, `provider_code`, `currency_code` |
...
## 7. Company/Client ID'leri (Core → Report DB'ler)

| Hedef DB | Tablolar | Alanlar |
|---|---|---|
| **Core Report** | `monthly_invoices`, `client_daily_kpi`, `client_traffic_hourly` | `company_id`, `client_id` |

---

## 8. Bonus Rule Snapshot (Bonus DB → Client)

| Akış | Tablo | Strateji |
|---|---|---|
| Bonus → **Client** | `bonus.bonus_awards` | `bonus_rule_id`, `bonus_type_code`, `bonus_subtype` + **`rule_snapshot` JSONB** (tüm kural immutable kopyası) |
...
sequenceDiagram
    ...
    BE->>Core: Auth kontrol + client bilgisi
    Core-->>BE: user, permissions, client
    BE->>Client: SELECT players (auth-agnostic)
    Client-->>BE: player listesi
    BE->>BE: In-memory enrichment<br/>(currency symbol, client name vb.)
...
```csharp
// 1. Core DB'den auth + client bilgisi
var user = await coreDb.GetUserWithPermissions(userId);
var client = await coreDb.GetClient(clientId);

// 2. Client DB'den oyuncu listesi (auth-agnostic)
var players = await clientDb.GetPlayers(page, filters);

// 3. Uygulama katmanında zenginleştirme
// currency_code zaten client DB'de var (denormalize)
...
public class ReferenceDataCache
{
    ...
    Dictionary<long, ClientDto> _clients;           // client_id → dto
    Dictionary<string, CountryDto> _countries;      // country_code → dto

    // Client DB'den gelen sonuçları zenginleştir
...
Master veri Core/Game/Finance DB'de değiştiğinde, outbox event ile Client DB'ye senkronize edilir.
...
sequenceDiagram
    ...
    Core->>Outbox: INSERT (cache_invalidate, client_games)
    Worker->>Outbox: Poll pending messages
    Worker->>Core: Güncel client_games oku
    Worker->>Client: game_settings UPDATE
    Note over Client: core_synced_at güncellenir
    Worker->>Outbox: status = completed
```

**Akış:**
1. **İlk provisioning:** Client oluşturulurken Core'daki `client_games`, `client_payment_methods` → Client DB'ye `game_settings`, `payment_method_settings` olarak kopyalanır
2. **Güncelleme:** BO'dan oyun/ödeme yöntemi değiştiğinde → Core güncellenir → Outbox event → Backend sync → Client güncellenir
3. **`core_synced_at`:** Her client tablosunda son sync zamanı tutulur
...
**Ne zaman:** `client_audit`, `core_audit`, `client_log`, `game_log` — tüm audit/log tabloları
...
- [PROVISIONING_GUIDE](PROVISIONING_GUIDE.md) — Client oluşturma ve ilk veri senkronizasyonu
