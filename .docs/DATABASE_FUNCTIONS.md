...
- **`ip_geo_cache_upsert(p_ip_address INET, p_country VARCHAR(100), p_country_code CHAR(2), p_continent VARCHAR(100), p_continent_code CHAR(2), p_region VARCHAR(100), p_region_name VARCHAR(200), p_city VARCHAR(200), p_district VARCHAR(200), p_zip VARCHAR(20), p_lat DECIMAL(9,6), p_lon DECIMAL(9,6), p_timezone VARCHAR(100), p_utc_offset INTEGER, p_currency VARCHAR(10), p_isp VARCHAR(300), p_org VARCHAR(300), p_as_number VARCHAR(200), p_as_name VARCHAR(300), p_reverse_dns VARCHAR(300), p_is_mobile BOOLEAN, p_is_proxy BOOLEAN, p_is_hosting BOOLEAN, p_ttl_days INT DEFAULT 30)`**: Upserts IP geo cache entry with full ip-api.com fields and configurable TTL. Returns VOID.
...
- **`platform_setting_create(p_category VARCHAR, p_setting_key VARCHAR, p_setting_value TEXT, p_environment VARCHAR DEFAULT 'production', p_description VARCHAR DEFAULT NULL)`**: Creates a new platform service configuration.
- **`platform_setting_delete(p_id BIGINT)`**: Soft deletes a platform service configuration by setting is_active to false.
- **`platform_setting_get(p_id BIGINT)`**: Returns a platform service configuration by ID.
...
- **`session_cleanup_expired(p_batch_size INT DEFAULT 1000, p_revoked_retention_days INT DEFAULT 7, p_inactivity_days INT DEFAULT 5)`**: Session cleanup expired. Uses PK-based delete for partitioned table compatibility.
...
#### Admin Functions (Draft, Publish, Recall, Send)
- **`admin_message_draft_create(p_caller_id BIGINT, p_sender_id BIGINT, p_subject VARCHAR(500), p_body TEXT, p_message_type VARCHAR(30) DEFAULT 'announcement', p_priority VARCHAR(10) DEFAULT 'normal', p_company_id BIGINT DEFAULT NULL, p_tenant_ids BIGINT[] DEFAULT NULL, p_department_id BIGINT DEFAULT NULL, p_role_id BIGINT DEFAULT NULL, p_scheduled_at TIMESTAMP DEFAULT NULL, p_expires_at TIMESTAMP DEFAULT NULL, p_created_by BIGINT DEFAULT NULL)`**: Creates a message draft with caller scope validation. Validates caller access to company_id and tenant_ids. Returns INTEGER (draft_id).
- **`admin_message_draft_update(p_caller_id BIGINT, p_draft_id INTEGER, p_subject VARCHAR(500) DEFAULT NULL, p_body TEXT DEFAULT NULL, p_message_type VARCHAR(30) DEFAULT NULL, p_priority VARCHAR(10) DEFAULT NULL, p_company_id BIGINT DEFAULT NULL, p_tenant_ids BIGINT[] DEFAULT NULL, p_department_id BIGINT DEFAULT NULL, p_role_id BIGINT DEFAULT NULL, p_scheduled_at TIMESTAMP DEFAULT NULL, p_expires_at TIMESTAMP DEFAULT NULL)`**: Updates a draft/scheduled message with caller scope validation. Validates caller access to company_id and tenant_ids. Returns BOOLEAN.
- **`admin_message_draft_get(p_draft_id INTEGER)`**: Returns draft details with read statistics for published drafts. Returns JSONB.
- **`admin_message_draft_list(p_sender_id BIGINT DEFAULT NULL, p_status VARCHAR(20) DEFAULT NULL, p_message_type VARCHAR(30) DEFAULT NULL, p_search VARCHAR(200) DEFAULT NULL, p_offset INTEGER DEFAULT 0, p_limit INTEGER DEFAULT 20)`**: Lists drafts with sender, status, type and search filters. Returns paginated JSONB.
- **`admin_message_draft_delete(p_draft_id INTEGER, p_deleted_by BIGINT)`**: Soft deletes a draft. Only draft/scheduled/cancelled can be deleted. Published drafts cannot be deleted (use recall). Returns BOOLEAN.
- **`admin_message_draft_cancel(p_draft_id INTEGER)`**: Cancels a draft/scheduled message (status → cancelled). Returns BOOLEAN.
- **`admin_message_publish(p_caller_id BIGINT, p_draft_id INTEGER)`**: Publishes a draft message with caller scope validation. Validates caller access to draft's company_id and tenant_ids. Resolves recipients using AND-combined filters. Raises error if no recipients matched. Returns INTEGER (recipient count).
- **`admin_message_recall(p_draft_id INTEGER)`**: Recalls a published message. Soft deletes all user_messages linked to the draft and sets draft status to cancelled. Only published drafts can be recalled. Returns INTEGER (affected message count).
- **`admin_message_send(p_sender_id BIGINT, p_recipient_id BIGINT, p_subject VARCHAR(500), p_body TEXT, p_message_type VARCHAR(30) DEFAULT 'direct', p_priority VARCHAR(10) DEFAULT 'normal', p_expires_at TIMESTAMP DEFAULT NULL)`**: Sends a direct message to a single user inbox. Validates recipient exists/active, prevents self-messaging, validates sender access to recipient company scope. No draft involvement. Returns message ID.

#### User Functions (Inbox)
- **`user_message_list(p_user_id BIGINT, p_is_read BOOLEAN DEFAULT NULL, p_priority VARCHAR(10) DEFAULT NULL, p_offset INT DEFAULT 0, p_limit INT DEFAULT 20)`**: Lists user inbox messages with read/priority filters. Excludes expired messages. Returns JSONB with total, unread_count and paginated items including draft_id.
- **`user_message_read(p_user_id BIGINT, p_message_id BIGINT)`**: Marks a message as read. Only the recipient can mark their own messages. Returns TRUE on success.
...
- **`create_partitions(p_days_ahead INT DEFAULT 7)`**: Creates daily partitions for core_log tables. Look-ahead: today + N days. Idempotent.
...
- **`partition_info()`**: Reports partition status for all partitioned tables in core_log. Shows count, size, oldest/newest partitions.
...
## Core Report Database

### Maintenance Schema

- **`create_partitions(p_months_ahead INT DEFAULT 3)`**: Creates monthly partitions for all report tables. Generates current month plus look-ahead months. Skips already existing partitions.
- **`drop_expired_partitions(p_retention_days INT DEFAULT NULL)`**: Drops expired monthly partitions older than retention period. Default: indefinite since this is business data.
- **`partition_info()`**: Returns detailed information about all partitions in performance, finance and billing schemas including row counts, sizes and partition ranges.
- **`run_maintenance(p_months_ahead INT DEFAULT 3, p_retention_days INT DEFAULT NULL)`**: Runs complete partition maintenance: creates future partitions and drops expired ones. Should be scheduled as a periodic job.
