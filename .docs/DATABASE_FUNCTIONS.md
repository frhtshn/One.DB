...
- **`admin_message_draft_create(p_sender_id BIGINT, p_subject VARCHAR(500), p_body TEXT, p_message_type VARCHAR(30) DEFAULT 'announcement', p_priority VARCHAR(10) DEFAULT 'normal', p_company_id BIGINT DEFAULT NULL, p_tenant_ids BIGINT[] DEFAULT NULL, p_department_id BIGINT DEFAULT NULL, p_role_id BIGINT DEFAULT NULL, p_scheduled_at TIMESTAMP DEFAULT NULL, p_expires_at TIMESTAMP DEFAULT NULL, p_created_by BIGINT DEFAULT NULL)`**: Creates a message draft. If scheduled_at provided, status='scheduled', otherwise 'draft'. tenant_ids supports multi-tenant targeting. Returns INTEGER (draft_id).
- **`admin_message_draft_update(p_draft_id INTEGER, p_subject VARCHAR(500) DEFAULT NULL, p_body TEXT DEFAULT NULL, p_message_type VARCHAR(30) DEFAULT NULL, p_priority VARCHAR(10) DEFAULT NULL, p_company_id BIGINT DEFAULT NULL, p_tenant_ids BIGINT[] DEFAULT NULL, p_department_id BIGINT DEFAULT NULL, p_role_id BIGINT DEFAULT NULL, p_scheduled_at TIMESTAMP DEFAULT NULL, p_expires_at TIMESTAMP DEFAULT NULL)`**: Updates a draft/scheduled message. Published drafts cannot be updated. NULL text params keep current value, filter fields always overwritten. Returns BOOLEAN.
...
- **`admin_message_publish(p_draft_id INTEGER)`**: Publishes a draft message. Reads content and filters from draft, resolves recipients using AND-combined filters (company/tenant_ids[]/department/role), creates individual user_messages with draft_id link. tenant_ids array supports multi-tenant targeting. Updates draft status to published. Returns INTEGER (recipient count).
- **`admin_message_recall(p_draft_id INTEGER)`**: Recalls a published message. Soft deletes all user_messages linked to the draft. Only published drafts can be recalled. Returns INTEGER (affected message count).
- **`admin_message_send(p_sender_id BIGINT, p_recipient_id BIGINT, p_subject VARCHAR(500), p_body TEXT, p_message_type VARCHAR(30) DEFAULT 'direct', p_priority VARCHAR(10) DEFAULT 'normal', p_expires_at TIMESTAMP DEFAULT NULL)`**: Sends a direct message to a single user inbox. No draft involvement. Returns message ID.
