# SORTIS ONE – LOG / AUDIT / BUSINESS

## Log, Audit ve Retention Stratejisi

Bu doküman, **Sortis One platformu** içinde üretilen **log, audit ve business (history)** verilerinin  
hangi veritabanında tutulacağını, ne kadar süre saklanacağını ve retention süresi sonunda  
nasıl temizleneceğini tanımlar.

**ÖNEMLİ:** Retention politikaları **statik değildir**. Client bazında, jurisdiction kurallarına (KYC/AML) göre  
**Dinamik Retention** uygulanır. Bu kurallar `core.client_data_policies` tablosunda saklanır.
...
| **client**         | Business & history (transactions, player_messages, wallets) | Monthly    | Sınırsız**      | -                 |
| **client_log**     | Client operasyonel log + KYC/messaging/game logları     | Daily           | 30–90 gün***    | DROP partition    |
| **client_audit**   | Client audit kayıtları (player_audit + affiliate_audit) | Hybrid****      | 365 gün–5 yıl   | DROP partition    |
| **client_report**  | Client'a özel raporlar ve istatistikler                 | Monthly         | Sınırsız        | -                 |
| **client_affiliate**| Affiliate tracking ve komisyon yönetimi                 | Monthly         | Sınırsız        | -                 |
...
>
> \*\*\* `client_log` schemaları: `affiliate_log` (3 tablo, 90 gün), `kyc_log` (1 tablo, 90+ gün), `messaging_log` (1 tablo, 90 gün), `game_log` (1 tablo — `game_rounds`, 30 gün). KYC provider logları için retention compliance gereksinimleri doğrultusunda 90+ güne uzatılabilir.
>
> \*\*\*\* `client_audit` Hybrid: `player_audit.login_attempts` Daily (365 gün), `player_audit.login_sessions` Monthly (5 yıl). Diğer tablolar (affiliate_audit, kyc_audit) partitioned değildir.

---

## 2. Dinamik Retention Yapısı (`client_data_policies`)

Sistemde her client'ın maruz kaldığı regülasyonlar (MGA, Curaçao, UKGC vb.) farklıdır.  
Bu nedenle log ve audit verilerinin saklanma süresi **Configurable Job** mantığıyla yönetilir.

### 2.1 Konfigürasyon Tablosu

`core.client_data_policies` tablosu, temizlik job'ları için kaynak görevi görür.

```sql
SELECT * FROM core.client_data_policies WHERE client_id = 123;
```

**Örnek Kayıtlar:**

| Client ID | Veri Tipi   | Saklama (Gün) | Süre Sonu Aksiyon | Açıklama                |
| --------- | ----------- | ------------- | ----------------- | ----------------------- |
...
```mermaid
flowchart TD
    A[Cron Job Başlar] --> B{Client ID Loop}
    B --> C[core.client_data_policies Oku]
...
- Veritabanından silinir (`core.client_data_policies.action_type = 'ARCHIVE_COLD'`).
...
3. **Cold Storage Upload**: Export edilen dosya S3, Azure Blob Storage veya Glacier gibi ucuz ve güvenli bir depolama alanına yüklenir. Dosya isimlendirmesinde `client_id`, `table_name` ve `date_range` mutlaka bulunmalıdır.
...
1.  **Config**: Ürün/Compliance ekibi client açılırken politikaları girer.
...
### 6.1 client DB (Business Data - Sınırsız)
...
### 6.2 client_audit DB (Compliance Audit - 5-10 yıl)
...
### 6.3 client_log DB (Operational Logs - 90+ gün)
