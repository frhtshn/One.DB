...
| `game_log`        | Oyun round/spin detay logları (yüksek hacim, per-tenant izole) | Daily     | 30 gün    |
...
Partitioned tablolar **kendi tablo dosyasında inline** tanımlıdır (`PARTITION BY RANGE` + `DEFAULT` partition). 10 veritabanında toplam 37 tablo partitioned çalışır.
