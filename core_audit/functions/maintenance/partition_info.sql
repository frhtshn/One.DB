...
-- core_audit veritabanı için monitoring ve health check
...
BEGIN
    ...
    WITH partitions AS (
        ...
        WHERE pn.nspname IN ('backoffice')
    ),
    summary AS (
        SELECT
            p.parent_tbl,
...
COMMENT ON FUNCTION maintenance.partition_info() IS 'Reports partition status for all partitioned tables in core_audit. Shows count, size, oldest/newest partitions.';
