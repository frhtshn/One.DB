...
-- core_audit veritabanı için retention yönetimi
-- Aktif (bugünün) partition'ı ASLA silmez
-- ================================================================
...
CREATE OR REPLACE FUNCTION maintenance.drop_expired_partitions(
    p_retention_days INT DEFAULT 90  -- Varsayılan retention: 90 gün
)
RETURNS TABLE (
    table_name TEXT,
    partition_name TEXT,
...
BEGIN
    ...
    FOR v_rec IN
        ...
        WHERE pn.nspname IN ('backoffice')
          AND cc.relname NOT LIKE '%_default'  -- Default partition'ı atla
        ORDER BY cc.relname
    LOOP
