...
CREATE OR REPLACE FUNCTION maintenance.run_maintenance(
    p_retention_days INT DEFAULT 90,   -- Retention suresi (gun)
    p_look_ahead_days INT DEFAULT 7    -- Kac gun ileri partition olustur
)
RETURNS TABLE (
    operation TEXT,
    table_name TEXT,
...
COMMENT ON FUNCTION maintenance.run_maintenance(INT, INT) IS 'Main maintenance function for cron jobs. Creates future partitions and drops expired ones in a single call.';
